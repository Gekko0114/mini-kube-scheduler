...
func deepEqualWithoutGeneration(actual *nodeInfoListItem, expected *waitingpod.NodeInfo) error {
...
func newNodeInfo(pods []*v1.Pod) *waitingpod.NodeInfo {
	nodeInfo := waitingpod.NewNodeInfo(pods...)
	return nodeInfo
}
...
func TestExpirePod(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			cleanupTime: now.Add(2 * defaultTTL),
			wNodeInfo:   newNodeInfo([]*v1.Pod{testPods[2], testPods[1]}),
			ttl:         defaultTTL,
		},
		{
			...
			wNodeInfo:   newNodeInfo([]*v1.Pod{testPods[0]}),
			ttl:         time.Duration(0),
		},
	}
...
func TestAddPodAlwaysUpdatesPodInfoInNodeInfo(t *testing.T) {
	...
	tests := []struct {
		...
		nodeInfo             map[string]*waitingpod.NodeInfo
	}{
		{
			podsToAssume:         []*v1.Pod{p1},
			podsToAddAfterAssume: []*v1.Pod{p2},
			nodeInfo: map[string]*waitingpod.NodeInfo{
				"node1": newNodeInfo([]*v1.Pod{p2}),
			},
		},
	}
...
func TestAddPodWillReplaceAssumed(t *testing.T) {
	...
	tests := []struct {
		...
		wNodeInfo map[string]*waitingpod.NodeInfo
	}{{
		...
		wNodeInfo: map[string]*waitingpod.NodeInfo{
			"assumed-node": nil,
			"actual-node":  newNodeInfo([]*v1.Pod{updatedPod.DeepCopy()}),
		},
	}}
...
func TestAddPodAfterExpiration(t *testing.T) {
	...
	tests := []struct {
		...
		pod:       basePod,
		wNodeInfo: newNodeInfo([]*v1.Pod{basePod}),
	}}
...
func TestUpdatePod(t *testing.T) {
	...
	tests := []struct {
		...
		wNodeInfo:    []*waitingpod.NodeInfo{newNodeInfo([]*v1.Pod{testPods[1]}), newNodeInfo([]*v1.Pod{testPods[0]})},
	}}
...
func TestRemovePod(t *testing.T) {
	...
	wNodeInfo := newNodeInfo([]*v1.Pod{pod})
...
func compareCacheWithNodeInfoSnapshot(t *testing.T, cache *cacheImpl, snapshot *Snapshot) error {
	...
	expectedNodeInfoList := make([]*waitingpod.NodeInfo, 0, cache.nodeTree.numNodes)
	expectedHavePodsWithAffinityNodeInfoList := make([]*waitingpod.NodeInfo, 0, cache.nodeTree.numNodes)
	expectedUsedPVCSet := sets.NewString()
	nodesList, err := cache.nodeTree.list()
	if err != nil {
		t.Fatal(err)
...
func buildNodeInfo(node *v1.Node, pods []*v1.Pod) *waitingpod.NodeInfo {
	expected := waitingpod.NewNodeInfo()
...
func makeBasePod(t testingMode, nodeName, objName, cpu, mem, extended string, ports []v1.ContainerPort) *v1.Pod {
	...
	pod := v1.Pod{}
	pod.SetName(objName)
	pod.SetNamespace("node_info_cache_test")
	pod.SetUID(types.UID(objName))
